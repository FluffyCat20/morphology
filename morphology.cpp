...
//sizeI*sizeJ - image size
{
	...
	if (flag)
	{
		return std::max(std::max (ru[up][right], rd[down][right]), std::max(lu[up][left], ld[down][left]));
	}
	else 
	{
		return std::min(std::min(ru[up][right], rd[down][right]), std::min(lu[up][left], ld[down][left]));
	}
}
...
int main(int argc, char*argv[])
{
	...
	VanHerkInit(img, 1, 5, 5, maxRD, maxRU, maxLD, maxLU);
	VanHerkInit(img, 1, 5, 5, minRD, minRU, minLD, minLU);

	cv::Mat result (img);
	delatation(result, 5, 5, maxRD, maxRU, maxLD, maxLU);
